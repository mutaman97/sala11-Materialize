...
function loadStaticData() {
    ...
    $.ajax({
        ...
        success: function(response){ 
            ...
            out_of_stock_products(response.out_of_stock_products);  
            // end added
           ...
           if ($('.pending_order_list').length != 0) {
            $.each(data,function(index,row){
                ...
                var html=`<div class="col-12 col-md-6 col-lg-3 orderitems"> <div class="card card-primary ">
                      ...
                      <div class="card-body">
                        <h6>Amount: ${amount_format(row.amount)}<br><br>
...
$.ajax({
  ...
  { 
    ...
    $.each(response, function(index, value){
      if (value == 'on') {
        var status= enabledText;
      }
      else if(value == 'off'){
        
          var status= disabledText;
      }
      else if(value == 'Unlimited'){
        
        var status= unlimitedText;
      }
      else{
          var status=value;
      };
      // Adde By Mutaman For Translation  
      if (index == 'Sub Domain') {
        var index= subDomain;
      }
      else if(index == 'Custom Domain'){
        var index= customDomain;
      }
      else if(index == 'Barcode'){
        var index= barCode;
      }
      else if(index == 'Qr Code'){ 
        var index= qrCode;
      }
      else if(index == 'Customer Modules'){
        var index= customerModules;
      }
      else if(index == 'Pos'){
        var index= pos;
      }
      else if(index == 'Push Notification'){
        var index= pushNotification;
      }
      else if(index == 'Image Optimization'){
        var index= imageOptimization;
      }
      else if(index == 'Custom Css Js'){
        var index= customCss_Js;
      }
      else if(index == 'Pwa'){
        var index= pwa;
      }
      else if(index == 'Storage Limit'){
        var index= storageLimit;
      }
      else if(index == 'Post Limit'){
        var index= postLimit;
      }
      else if(index == 'Staff Limit'){
        var index= staffLimit;
      }          
      else{
        var index=index;
      };
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                pointHoverBackgroundColor: '#7952b3',
            }]
        },
        options: {
            legend: {
...
balance_chart_bg_color.addColorStop(0, 'rgba(121,82,179,.2)');
balance_chart_bg_color.addColorStop(1, 'rgba(121,82,179,0)');
     ...
     var html=`<div>
            <div class="product-item pb-3">
              <div class="product-image">
                <a href="${base_url+'/seller/user/'+row.id}">
                  <img alt="image" src="${gravatar(row.email)}" class="img-fluid">
                </a>
              </div>
              <div class="product-details">
                ...
                <div class="product-cta">
                  <a href="${base_url+'/seller/user/'+row.id}" class="btn btn-primary">${productDetailsText}</a>
                </div>
              </div>  
            </div>
          </div>`;
...
function top_sell_products(data) {
  $.each(data,function (index, row) {
     ...
     var html=`<div>
            <div class="product-item pb-3">
              <div class="product-image">
              <a href="${base_url+'/seller/product/'+row.id+'/edit'}">
              ...
              <div class="product-details">
                ...
                <div class="product-cta">
                  <a href="${base_url+'/seller/product/'+row.id+'/edit'}" class="btn btn-primary">${productDetailsText}</a>
                </div>
              </div>  
            </div>
          </div>`;
...
// added by mutaman for out of stock products
function out_of_stock_products(data) {
  $.each(data,function (index, row) {
       var ratings='';
     ...
     $('.out_of_stock_products').append(html);
  });
}
// end added
...
// Old Not Optimized Amount Format Function Disabled By Mutaman
...
// New Optimized Amount Format Function Added By Mutaman
