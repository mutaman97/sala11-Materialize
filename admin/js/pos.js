(function ($) {
  ...
  // disabled by mutaman coz the value of #currency name is not sent from layouts/app.blade.php
  // var currency_name= $('#currency_name').val();
  // end
  var currency_settings= $('#currency_settings').val();
  ...
  $('.barcode').on('input',function(){
    ...
    if (code != '') {
      ...
      success: function(response){ 
        ...
        if (response.product_id != '') {
          ...
          $('.subtotal').html(amount_format(response.cart_subtotal));
          $('.tax').html(amount_format(response.cart_tax));
          $('.total').html(amount_format(response.cart_total));
          $('.total_amount').val(amount_format(response.cart_total));
          audio();
    ...
    $(document).on('click','.add_to_cart',function(){
      ...
      if (stockmanage == 1) {
        var totalqty=$('.cart_qty'+id).val();
        totalqty=parseInt(totalqty);
        if (totalqty > stockqty) {
          Sweet('error', stockLimitExceeded);
          return true;
        }
      }
      
      if (stockstatus == 0) {
        Sweet('error', stockOut);
  ...
  <span class="visually-hidden"></span>
    ...
    function render_product(items) {
        ...
        $.each(items, function (key, item) 
        {
            ...
            // Changed by mutaman to show the currency icon in POS page
            // var price=amount_format(price);
            ...
            if (item.is_variation == 1) {
              var cart_html=`<a data-toggle="modal" data-target="#product_variation_modal" class="add_to_cart_modal" href="javascript:void(0)" data-isvariation="${item.is_variation}" data-id="${item.id}" ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M4 16V4H2V2h3a1 1 0 0 1 1 1v12h12.438l2-8H8V5h13.72a1 1 0 0 1 .97 1.243l-2.5 10a1 1 0 0 1-.97.757H5a1 1 0 0 1-1-1zm2 7a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm12 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"/></svg>${cartText}</a>`;
            }
            else{
              if(item.firstprice.stock_status == 0){
                var is_disable="disabled";
                var cart_text=`<del>${stockOut}</del>`;
              }
              else{
                var is_disable="";
                var cart_text= cartText;
              }

              // var cart_html=`<a ${is_disable} class="add_to_cart cart${item.id}" href="javascript:void(0)" data-isvariation="${item.is_variation}"  data-id="${item.id}" data-stockstatus="${item.firstprice.stock}" data-stockmanage="${item.firstprice.stock_manage}" data-qty="${item.firstprice.qty}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M4 16V4H2V2h3a1 1 0 0 1 1 1v12h12.438l2-8H8V5h13.72a1 1 0 0 1 .97 1.243l-2.5 10a1 1 0 0 1-.97.757H5a1 1 0 0 1-1-1zm2 7a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm12 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"/></svg>${cart_text}</a>`;
            ...
            var product_card=` <div class="col-lg-4 col-md-6 col-sm-12 mb-30 product p-2" >
                                  <div class="single-product shadow-sm">
                                    ...
                                    <div class="product-codebar">
                                      <span>${productCode} : ${item.full_id}</span>
                                    </div>
                                  </div>
                                </div>`;
    ...
    function amount_format(amount, type='name') {
      ...
      let format = type === 'name' ? ` ${currency_settings.currency_name} ` : currency_settings.currency_icon;
      
      // Check if currency format is SDG and replace it with sdgText if true
      if (format.trim() === 'SDG') {
        format = sdgText;
      }
      
      const price = currency_settings.currency_position === 'left' ? `${format} ${amount}` : `${amount} ${format}`;
        ...
        $.each(items, function (key, item) 
        {
          ...
          var cart_item=`<tr class="cart_item cart_item${item.rowId}">
            ...
            var cart_modal_item=`<tr class="cart_item cart_item${item.rowId}">
                  ...
                  <td class="text-right w-10">${amount_format(item.price)}</td>
               </tr>`;
  ...
  $(document).on('submit','.option_form', function(e) {
        ...
        if (required == true) {
          ...
          $.ajax({
            ...
            dataType: 'json',
            beforeSend: function(){
              $('.add_to_cart_form_btn').text(waitText);
              $('.add_to_cart_form_btn').attr('disabled','');
            },
            success: function(response){ 
              $('.add_to_cart_form_btn').text(CartTest);
            ...
            error: function(xhr, status, error) 
            {
              $('.add_to_cart_form_btn').text(CartText);
              $('.add_to_cart_form_btn').removeAttr('disabled');
