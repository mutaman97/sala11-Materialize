...
$('#locations').on('change',function(){
	...
	$.each(shippings,function(key,value){
		...
		var html=`<div class="form-group shipping_method">
						<div class="custom-control custom-checkbox">
							<input type="radio" class="custom-control-input shipping_item" name="shipping_method" value="${value.id}" data-price="${value.slug}" id="shipping${value.id}">
							<label for="shipping${value.id}" class="custom-control-label">${value.name}</label>
						</div>
					</div>`;
...
if ($(this).is(':checked')){
		$('.pre_order_area').removeClass('d-none');
	}
	else{
		$('.pre_order_area').addClass('d-none');
	}
});
